...
int main(int argc, char* argv[])
{
	...
	cudaMallocManaged((void**)&arr, (lines * 3) * sizeof(int));
	int counter = 0;
	int lineCounter = 0;
	...
	cudaMallocManaged((void**)&file_output_cuda, lines * sizeof(int));
	...
	operation << < num_of_blocks, 1024 >> > (arr, file_output_cuda, counter);
	cudaDeviceSynchronize();

	cudaEventRecord(stop, 0);
	cudaEventSynchronize(stop);
	cudaEventElapsedTime(&memsettime, start, stop);
	printf(" * CUDA execution time unified memory: %f * \n", memsettime);
